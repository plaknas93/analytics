...
tdata3 <- as(itemlist, "transactions")##converts into transactions
tdata3
summary(tdata3)
...
inspect(freqitems) #19 combinations against 9 transactions. Repeats itself in every combinations hence more 
freqitems = eclat(tdata, parameter = list(minlen=1, supp=.1, maxlen=2 ))
freqitems
inspect(freqitems)

support(items(freqitems[1:2]), transactions=tdata)
inspect(freqitems[1]) #inspect individual item only the first one
inspect(items(freqitems[1]))

itemFrequencyPlot(tdata,topN = 5,type="absolute")
itemFrequencyPlot(tdata,topN = 5,type="relative", horiz=T) #rel done horizontally
...
inspect(rules[c(8,9,10,11,12,13,14)])
