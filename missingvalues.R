...
sum(is.na(v1))  # sum TRUE values ie missing values. Total missing val
...
v1[is.na(v1)]
v1[is.na(v1)] = mean(v1, na.rm=T) #Assign mean val to NA
v1
...
sum(complete.cases(sleep)) # count non NA rows/ complete 42. sum count T
...
colSums(is.na(sleep))  #Total NA values in each column
rowSums(is.na(sleep))  #Total NA values in each row
head(sleep)
