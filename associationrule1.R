...
#Find Frequent Itemset #eclat is most freq purchased items indi/ together#Support analysis
frequentItems = eclat (Groceries)
inspect(frequentItems)
frequentItems = eclat (Groceries, parameter = list(supp = 0.01, minlen= 2, maxlen = 5))  #Min length indicates min basket size example 2 items together
inspect(frequentItems)
frequentItems
...
rules
inspect (rules[1:5]) #LHS can have multiple items but RHS single item
...
rules3 = apriori (data=Groceries, parameter=list (supp=0.002,conf = 0.8), appearance = list (default="lhs",rhs="whole milk"), control = list (verbose=F)) ##Verbose reduces the o/p content only to products 
inspect(rules3[1:5])
inspect(rules3)
...
inspect(subset1)
inspect(rules2)
subset2 = subset(rules2, subset=lhs %ain% c('whole milk','curd cheese') )
inspect(subset2)#subset2 for both item together
subset2a = subset(rules2, subset=lhs %in% c('whole milk','curd cheese') )
inspect(subset2a)#subset2a for either of the two items

rules3b = apriori(data=Groceries,parameter=list(supp=.002,conf=.7),appearance = list(default="lhs",rhs="whole milk"))
inspect(rules3b[1:5])
inspect(rules3b)
...
plot(subset4[1:10]) 
plot(subset4[1:10], measure=c("support", "lift"), shading="confidence")
